  ...
  const handleGuardarPago = () => {
    ...
    axios
      .post("https://apidev.gocastgroup.com/api/bnc/p2p.php", {
        ...
        clienteNombre: "VivirSeguros", // Asegúrate de que esté entre comillas
      })
      .then((response) => {
        ...
        if (response && response.data.status === "success") {
          ...
          handlePaymentData({
            fecha: new Date(),
            referencia: response.data.reference || referencia, // fallback por si no se devuelve
            monto: monto,
            banco: selectedBanco,
          });
          ...
          setPagoMovilResponse({
            ...
            additionalMessage: "Hemos enviado la factura a tu correo.",
            reference: response.data.reference || "Referencia no proporcionada",
          });
        } else {
          console.error("Error en la transacción:", response.data.message || "Error desconocido.");
        }
      })
      .catch((error) => {
        setPagoMovilResponse({
          message: "Error en tu transacción",
          additionalMessage: error.response?.data?.message || "No se pudo completar el pago",
          reference: "N/A",
        });
              ...
              <div id="modal-pagomovil" className="modal">
                <div className="modal-content">
                  ...
                  {pagoMovilResponse ? (
                     ...
                     <p>{pagoMovilResponse.message}
                     {pagoMovilResponse.additionalMessage}
                     {pagoMovilResponse.reference}
                     </p>
